...
if (!defined('RPT_CACHE_EXPIRATION')) {
    define('RPT_CACHE_EXPIRATION', 3600); // Example value, adjust as needed
}
...
class RelatedPostsThumbnails {
	...
	*/
	...
	// Define your plugin constants here
	function initialize_plugin_constants() {
		// Check if the constant is not already defined
		if (!defined('RPT_CACHE_EXPIRATION')) {
			// Define the constant with the desired value
			define('RPT_CACHE_EXPIRATION', 3600); // Example value, adjust as needed
		}
	}
	// Now you can proceed with the rest of your plugin code
	// Your other plugin code goes here...
	
	function get_html($show_top = false, $posts_number = '', $sort_by = '', $main_title = '', $exclude = '')
	{
		// Generate a unique cache key based on function arguments
		$cache_key = 'rpt_cache_' . md5(serialize(func_get_args()));
	
		// Check if caching is enabled and if there's cached data available
		if ($this->devmode == '0' && false !== ($cached_html = get_transient($cache_key))) {
			// Return cached data if available
			return $cached_html;
		}
	
		// If cached data is not available or caching is disabled, generate thumbnails as usual
		$html = ''; // HTML markup for related posts thumbnails
		// ... code to generate HTML markup for related posts thumbnails ...
	
		// Store the generated HTML markup in the cache
		if ($this->devmode == '0') {
			error_log('Cache Key: ' . print_r($cache_key, true));
			error_log('HTML Content: ' . print_r($html, true));
			error_log('Cache Expiration: ' . print_r(RPT_CACHE_EXPIRATION, true));
	
			set_transient($cache_key, $html, RPT_CACHE_EXPIRATION); // Store in cache with expiration time
		}
...
// Add a setting in the General tab to enable/disable category links
function custom_enable_category_links_settings() {
    add_settings_field(
        'enable_category_links',
        __('Enable Category Links in Related Posts', 'text-domain'),
        'custom_enable_category_links_callback',
        'general'
    );

    register_setting('general', 'enable_category_links');
}
add_action('admin_init', 'custom_enable_category_links_settings');

// Callback function to display the checkbox for enabling/disabling category links
function custom_enable_category_links_callback() {
    $enable_category_links = get_option('enable_category_links');
    ?>
    <label for="enable_category_links">
        <input type="checkbox" name="enable_category_links" id="enable_category_links" value="1" <?php checked(1, $enable_category_links); ?>>
        <?php _e('Enable category links in related posts', 'text-domain'); ?>
    </label>
    <?php
}

// Modify the category list to add links if enabled
function custom_modify_category_list($category_list, $id, $taxonomy, $post_type) {
    $enable_category_links = get_option('enable_category_links');
    
    // Check if category links are enabled
    if ($enable_category_links) {
        // Ensure $category_list is a string
        if (is_array($category_list)) {
            $category_list = wp_list_pluck($category_list, 'name');
            $category_list = implode('</span>', $category_list);
        }
        // Here, you can modify the $category_list to add links
        // Example: Add links to category archives
        // $category_list = str_replace('</span>', '</span>', $category_list);
    }
    
    return $category_list;
}
add_filter('relpoststh_show_all_categories', 'custom_modify_category_list', 10, 4);
...
// Step 1: User Interface - Add custom meta box for entering related posts from external websites
function add_external_related_posts_meta_box() {
    add_meta_box(
        'external-related-posts-meta-box',
        'External Related Posts',
        'render_external_related_posts_meta_box',
        'post',
        'normal',
        'default'
    );
}
add_action('add_meta_boxes', 'add_external_related_posts_meta_box');

function render_external_related_posts_meta_box($post) {
    // Display input fields for entering external related post details
    echo '<label for="external_post_url">Post URL:</label>';
    echo '<input type="text" id="external_post_url" name="external_post_url" value=""><br>';
    echo '<label for="external_post_title">Title:</label>';
    echo '<input type="text" id="external_post_title" name="external_post_title" value=""><br>';
    // Add more fields as needed (e.g., description, thumbnail)

    // Add nonce field for security
    wp_nonce_field('save_external_related_posts', 'external_related_posts_nonce');
}

// Step 2: Save External Related Posts
function save_external_related_posts($post_id) {
    // Check if nonce is set and valid
    if (!isset($_POST['external_related_posts_nonce']) || !wp_verify_nonce($_POST['external_related_posts_nonce'], 'save_external_related_posts')) {
        return;
    }
    ...
    // Save external related post details
    if (isset($_POST['external_post_url'])) {
        update_post_meta($post_id, 'external_post_url', sanitize_text_field($_POST['external_post_url']));
    }
    if (isset($_POST['external_post_title'])) {
        update_post_meta($post_id, 'external_post_title', sanitize_text_field($_POST['external_post_title']));
    }
    // Save more fields as needed
}
add_action('save_post', 'save_external_related_posts');

// Step 3: Fetch and Display External Related Posts
function display_external_related_posts($related_post_ids) {
    foreach ($related_post_ids as $post_id) {
        $external_post_url = get_post_meta($post_id, 'external_post_url', true);
        $external_post_title = get_post_meta($post_id, 'external_post_title', true);

        // Output the related post HTML
        echo '<div class="external-related-post">';
        echo '<a href="' . esc_url($external_post_url) . '">' . esc_html($external_post_title) . '</a>';
        echo '</div>';
    }
}

// Step 4: Modify the existing related posts display to include external related posts
function modify_related_posts_display($related_post_ids) {
    // Display local related posts
    display_local_related_posts($related_post_ids);

    // Display external related posts
    display_external_related_posts($related_post_ids);
}
add_action('relpoststh_display_related_posts', 'modify_related_posts_display');
